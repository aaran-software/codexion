...
function HeaderPortfolio() {
    ...
    return (
        <div
            className='bg-background flex flex-row justify-between h-20 items-center px-5 py-2 md:py-4 w-full fixed top-0 left-0 z-50 shadow-md'>
            {/* Logo */}
            ...
            <ul className='hidden md:flex flex-row justify-between gap-10 items-center'>
                {menu.map((item, index) => (
                    <li key={index} className='relative group'>
                        ...
                        <span
                            className='absolute left-0 bottom-0 h-0.5 w-full transform scale-x-0 origin-left group-hover:scale-x-100 group-hover:bg-[#6ab48d] transition-transform duration-100 ease-in-out'></span>
                    </li>
                ))}
            </ul>
