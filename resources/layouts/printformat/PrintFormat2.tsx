...
function PrintFormat2({
  ...
  return (
    <div className="border border-ring w-full text-[10px] overflow-x-auto">
      {/* header */}
      ...
      <table className="min-w-full table-fixed border-b border-ring">
        <thead>
          <tr>
            ...
            {shouldShowTotal && (
              <th className="border border-ring first:border-l-0 last:border-r-0 px-2 py-1 text-center font-semibold w-[120px]">
                Total
              </th>
            )}
          </tr>
        ...
        <tbody>
          {(() => {
            const maxLines = 25; // total printable lines
            let usedLines = 0;
            const rowsToRender: JSX.Element[] = [];

            // find Item Name index
            const itemNameIndex = head.findIndex((h) =>
              h.toLowerCase().includes("item")
            );
            const charsPerLine = 30; // tweak to match your column width & font

            for (let rowIndex = 0; rowIndex < computedBody.length; rowIndex++) {
              const row = computedBody[rowIndex];
              if (!row) continue;

              const itemName = row[itemNameIndex] || "";

              // --- estimate required lines for this row ---
              const estimatedLines =
                Math.ceil(itemName.length / charsPerLine) || 1;

              // if this product does not fit fully, stop rendering
              if (usedLines + estimatedLines > maxLines) break;

              // otherwise include this product
                ...
                <tr key={rowIndex} className="align-top">
                  {head.map((h, i) => {
                    ...
                    return (
                      <td
                        key={i}
                        className={`border-x border-ring px-1 py-0 leading-tight text-${align} first:border-0 last:border-0 ${columnWidths[h] || "w-auto"}`}
                      >
                        {row?.[i] || ""}
                      </td>
                    );
                  ...
                  {shouldShowTotal && (
                    <td className="border-x border-ring px-1 py-0 leading-tight text-right first:border-0 last:border-0 w-[120px]">
                      {row?.[row.length - 1] || ""}
                    </td>
                  )}
                </tr>
