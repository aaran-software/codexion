...
const categories = ["all", "website", "app"] as const;
...
const ProductShowcase: React.FC<Props> = ({ products }) => {
  ...
  const filteredProducts =
    ...
    const [filerref1, inView1] = useInView({ triggerOnce: false, threshold: 0.2 });
    const [filerref2, inView2] = useInView({ triggerOnce: false, threshold: 0.2 });
      ...
      <div ref={filerref1} className="absolute top-4 left-4 flex gap-3">
        {categories.map((cat,index) => (
          <button
            ...
            className={`px-4 py-1 rounded-full text-sm font-medium shadow-md transition cursor-pointer animate__animated ${inView1 ? "animate__fadeInRight" : "opacity-0"} ${
              activeCategory === cat
  ...
  {categories.map((cat, index) => {
    const reverseIndex = categories.length - 1 - index; // reverse order
    return (
      <button
        ref={filerref2}
        key={cat}
