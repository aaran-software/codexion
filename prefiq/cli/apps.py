...
from prefiq.apps.app_cfg import get_registered_apps, get_version, load_cfg, get_cfg_abspath
...
def cmd_list() -> None:
    ...
    and display which config file is being read.
    """
    cfg_file = get_cfg_abspath()
    names = get_registered_apps()
    if not names:
        typer.echo(f"No apps registered yet.\n(using {cfg_file})")
        return
