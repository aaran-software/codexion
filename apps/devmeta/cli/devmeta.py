...
def _ensure_devmeta_bound():
    """
    Ensure the DevMeta provider has bound the migrator and services.
    Returns the DI container (Application.get_app()).
    """
    ...
    # Try to import provider and register/boot it
    DevMetaProvider = None
    try:
    ...
    if DevMetaProvider is not None:
        ...
        try:
            ...
            LOG.warning("devmeta_register_failed", extra={"error": repr(e)})
        try:
            prov.boot(appc)
        except Exception as e:
            LOG.warning("devmeta_boot_failed", extra={"error": repr(e)})

    return appc


# ---------------------------------------------------------------------------
# Typer app & mount helpers
# ---------------------------------------------------------------------------
...
def mount_devmeta_cli(container) -> bool:
    ...
    try:
        ...
        if not mounted:
            LOG.info("cli_mount_skipped", extra={"reason": "host lacks add_typer/mount_subapp"})
            return False
...
def cmd_rollback(steps: int = typer.Option(1, "--steps", help="Number of migrations to roll back")):
    """Roll back the last N DevMeta migrations."""
    appc = _ensure_devmeta_bound()
    migrator = appc.resolve("devmeta.migrator")
