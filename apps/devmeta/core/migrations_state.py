...
DEFAULT_TABLE_NAME = "devmeta_migrations"
...
def ensure_devmeta_migrations_table(
    ...
    ddl_table = _DDL_CREATE_TABLE.replace(DEFAULT_TABLE_NAME, table_name)
    ddl_indexes = [stmt.replace(DEFAULT_TABLE_NAME, table_name) for stmt in _DDL_CREATE_INDEXES]
...
def get_applied_migration_names(
    ...
    # rows may be tuples or Row objects depending on row_factory
    def _get_name(r): return r["name"] if hasattr(r, "__getitem__") and not isinstance(r, tuple) else r[0]
    return { _get_name(r) for r in rows }
